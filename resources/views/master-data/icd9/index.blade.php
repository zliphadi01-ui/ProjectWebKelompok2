...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="text-primary fw-bold"><i class="bi-journal-medical me-2"></i> Master Data ICD-9</h2>
    <div>
...
<div class="card shadow border-0 mb-4">
    <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
        <h6 class="m-0 fw-bold text-primary">Daftar Kode Tindakan (ICD-9)</h6>
        <form action="{{ route('master-data.icd9.index') }}" method="GET" class="d-flex">
    ...
    <div class="card-body p-0">
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    @forelse($procedures as $procedure)
                        <tr>
                            <td class="ps-4 fw-bold text-primary">{{ $procedure->code }}</td>
                            <td>{{ $procedure->name }}</td>
                            <td class="text-end pe-4">
                                <button class="btn btn-sm btn-outline-warning rounded-pill" 
                                        data-bs-toggle="modal" 
                                        data-bs-target="#editModal{{ $procedure->id }}">
                                    <i class="bi-pencil"></i>
                                </button>
                                <form action="{{ route('master-data.icd9.destroy', $procedure->id) }}" method="POST" style="display:inline-block" onsubmit="return confirm('Hapus kode {{ $procedure->code }}?')">
                                    @csrf
                                    @method('DELETE')
                        ...
                        {{-- EDIT MODAL --}}
                        <div class="modal fade" id="editModal{{ $procedure->id }}" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title">Edit ICD-9</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <form action="{{ route('master-data.icd9.update', $procedure->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <div class="modal-body">
                                            <div class="mb-3">
                                                <label class="form-label">Kode ICD-9</label>
        ...
        <div class="card-footer bg-white">
            {{ $procedures->appends(['q' => $query])->links() }}
        </div>
        @endif
    </div>
</div>
...
<div class="modal fade" id="addModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Tambah ICD-9 Manual</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('master-data.icd9.store') }}" method="POST">
                @csrf
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label">Kode ICD-9</label>
                        <input type="text" name="code" class="form-control" placeholder="Contoh: 00.01" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Nama Tindakan</label>
                        <input type="text" name="name" class="form-control" placeholder="Contoh: Therapeutic ultrasound" required>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
...
<div class="modal fade" id="importModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Import CSV ICD-9</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('master-data.icd9.import') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
